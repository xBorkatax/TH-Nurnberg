Start(out int zeilen, out int spalten);
FieldCreate(zeilen, spalten, out int[][] field);
int countForPlayer = 0; 
Play(field, ref countForPlayer);

static void Start(out int zeilen, out int spalten )
{
    zeilen = 0;
    spalten = 0;
...
static void FieldCreate(int zeilen, int spalten, out int[][] field) //spalten kolona
{
    field = new int[spalten][];
    for (int i = 0; i < spalten; i++)
    {
        field[i] = new int[zeilen];
    }
}
...
static bool CheckWinner(int[][] field, int playerSymbol)
{
    ...
    for (int i = field.Length; i > field.Length - 2; i--)
    {
        for (int n = field[i - 1].Length; n > field[i - 1].Length - 2; n--)
        {
            if (field[i - 1][n - 2] == playerSymbol && field[i - 2][n - 1] == playerSymbol && field[i - 3][n] == playerSymbol)
            {
                return true;
            }
        }
